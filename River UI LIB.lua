	...
	function uitab:Tab(text)
		...
		function main:SliderToggle(text,config, min, max, set, callback)
			...
			AHEHE.MouseButton1Down:Connect(
				function()
					Value =
						math.floor((((tonumber(max) - tonumber(min)) / 315) * bar1.AbsoluteSize.X) + tonumber(min)) or 0
						pcall(
							function()
								callback(Value,toggled)
							end
					...
					moveconnection =
						...
						function()
							...
							Value =
								math.floor(
								(((tonumber(max) - tonumber(min)) / 315) * bar1.AbsoluteSize.X) + tonumber(min)
							)
							pcall(
								function()
									callback(Value,toggled)
							...
							bar1.Size = UDim2.new(0, math.clamp(mouse.X - bar1.AbsolutePosition.X, 0, 315), 0, 5)
							circlebar.Position =
								UDim2.new(0, math.clamp(mouse.X - bar1.AbsolutePosition.X - 2, 0, 315), 0, -3)
						end
					)
					releaseconnection =
						uis.InputEnded:Connect(
